mGBA with Multiplayer
====
mGBA 를 웹으로 포팅하고, 호스트가 참여자 수만큼 최대 4개의 에뮬레이터를 실행합니다. 참여자는 자신에게 해당하는 에뮬레이터의 화면과 소리를 공유받고 키 입력을 호스트에게 보냅니다. 호스트는 참여자로부터 수신한 키 입력을 해당하는 에뮬레이터에 입력합니다. 이렇게 하면 네트워크 지연에 의한 연결 끊김 현상을 제거할 수 있습니다. 참여자는 약간의 비디오 디코딩, 입력 지연을 겪을 수 있습니다.

안내 및 법적 고지 (Disclaimer)
===
본 서비스는 mGBA 에뮬레이터의 웹 포팅 버전으로, 어떠한 게임 파일(ROM)도 내장하거나 제공하지 않습니다.

본 에뮬레이터는 사용자가 직접 제작한 홈브루(Homebrew) 소프트웨어, 오픈소스 게임, 또는 합법적으로 소유한 원본 카트리지의 백업본을 구동하기 위한 도구입니다.

저작권자의 허가를 받지 않은 상용 게임 ROM 파일을 무단으로 다운로드하여 구동하는 행위는 본 프로젝트의 취지와 무관하며, 이로 인해 발생하는 모든 법적 책임은 전적으로 사용자 본인에게 있습니다.

원격 플레이 시 발생하는 데이터 송신 및 저작권 책임은 전적으로 세션을 개설한 사용자에게 있습니다.
