...
====
mGBA 를 웹으로 포팅하고, 호스트가 참여자 수만큼 최대 4개의 에뮬레이터를 실행합니다. 참여자는 자신에게 해당하는 에뮬레이터의 화면과 소리를 공유받고 키 입력을 호스트에게 보냅니다. 호스트는 참여자로부터 수신한 키 입력을 해당하는 에뮬레이터에 입력합니다. 이렇게 하면 네트워크 지연에 의한 연결 끊김 현상을 제거할 수 있습니다. 참여자가 약간의 비디오 디코딩, 입력 지연을 겪을 수 있습니다만 우리는 그 정도 지연과 흐릿해짐에 익숙합니다.

아래는 mGBA 프로젝트 README 원문입니다
